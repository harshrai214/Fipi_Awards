...
//       attachments1_desc: "t4rew",
//       attachments1: "gh/attachments/Daily_Task_2024-10-17_121833.png",
//       attachments2_desc: "rewf",
//       attachments2: "gh/attachments/Daily_Task_2024-10-30_121833.png",
//       attachments3_desc: "efew",
//       attachments3: "gh/attachments/Daily_Task_2024-11-07_121833.png",
//       attachments4_desc: "cewvr",
//       attachments4: "gh/attachments/Daily_Task_2024-10-15_121833.png",
//       created_at: "2025-09-01T18:14:20.202166",
//       user_id: 1,
...
//       attachments4_desc: "",
//       attachments4: "",
//       created_at: "2025-09-04T18:02:58.358946",
//       user_id: 3,
...
//       attachments4_desc: "",
//       attachments4: "",
//       created_at: "2025-09-04T18:46:43.024001",
//       user_id: 2,
...
//   const companyData = staticData.reduce((acc, item) => {
//     if (companies.includes(item.organisation_name)) {
//       acc[item.organisation_name] = item;
//     }
//     return acc;
//   }, {});
...
//   const getValues = (key) =>
//     companies.map((c) => Number(companyData[c]?.[key] || 0));

//   const getGrowth = (keyCurrent, keyPrevious) =>
//     companies.map((c) => {
//       const curr = Number(companyData[c]?.[keyCurrent] || 0);
//       const prev = Number(companyData[c]?.[keyPrevious] || 0);
//       if (prev > 0) {
...
//   const calculateHighestScore = (values, maxScore) => {
//     const finiteValues = values.filter((v) => isFinite(v) && v > 0);
//     const maxVal = finiteValues.length > 0 ? Math.max(...finiteValues) : 0;
//     if (maxVal <= 0) return companies.map(() => 0);
//     return values.map((v) =>
//       isFinite(v) && v > 0 ? (v / maxVal) * maxScore : 0
//     );
//   };

//   const calculateLowestScore = (values, maxScore) => {
//     const finiteValues = values.filter((v) => isFinite(v) && v > 0);
//     const minVal = finiteValues.length > 0 ? Math.min(...finiteValues) : 0;
//     if (minVal <= 0) return companies.map(() => 0);
//     return values.map((v) =>
//       isFinite(v) && v > 0 ? (minVal / v) * maxScore : 0
//     );
//   };

//   const val11 = getValues("installed_capacity_2024");
//   const val12 = getValues("production_2024");
//   const val13 = getValues("carbon_emission_2024");
//   const val14 = getValues("purity_2024");
//   const val15 = getValues("cost_of_production_2024");
//   const val21 = getValues("investment_activities_2024");
//   const val22 = getGrowth("investment_activities_2024", "investment_activities_2023");
//   const val31 = getValues("investment_electrolyser_2024");
//   const val32 = getGrowth("investment_electrolyser_2024", "investment_electrolyser_2023");
//   const val41 = getValues("patents_filed_2024");
//   const val42 = getValues("patents_granted_national_2024");
//   const val43 = getValues("patents_granted_international_2024");
//   const val44 = getValues("patents_commercialized_2024");
//   const val51 = companies.map((c) => {
//     const cap = Number(companyData[c]?.capacity_1 || 0);
//     const year = Number(companyData[c]?.project_completion_year_1 || 0);
//     if (cap > 0 && year > currentYear) {
//       return (year - currentYear) / cap;
//     }
//     return Infinity;
//   });
//   const val52 = getValues("capacity_1"); 

//   const score11 = getValues("installed_capacity_2024_score");
//   const score12 = getValues("production_2024_score");
//   const score13 = getValues("carbon_emission_2024_score");
//   const score14 = getValues("purity_2024_score");
//   const score15 = getValues("cost_of_production_2024_score");
//   const score21 = calculateHighestScore(val21, 10);
//   const score22 = calculateHighestScore(val22, 10);
//   const score31 = calculateHighestScore(val31, 5);
//   const score32 = calculateHighestScore(val32, 5);
//   const score41 = getValues("patents_filed_2024_score");
//   const score42 = getValues("patents_granted_national_2024_score");
//   const score43 = getValues("patents_granted_international_2024_score");
//   const score44 = getValues("patents_commercialized_2024_score");
//   const score51 = calculateLowestScore(val51, 10);
//   const score52 = calculateHighestScore(val52, 10);

//   const section1 = companies.map(
//     (_, i) => score11[i] + score12[i] + score13[i] + score14[i] + score15[i]
//   );
//   const section2 = companies.map((_, i) => score21[i] + score22[i]);
//   const section3 = companies.map((_, i) => score31[i] + score32[i]);
//   const section4 = companies.map(
//     (_, i) => score41[i] + score42[i] + score43[i] + score44[i]
//   );
//   const section5 = companies.map((_, i) => score51[i] + score52[i]);
//   const grandTotal = companies.map(
//     (_, i) => section1[i] + section2[i] + section3[i] + section4[i] + section5[i]
//   );
//   const projectTime = companies.map((c) => {
//     const year = Number(companyData[c]?.project_completion_year_1 || 0);
//     return year > currentYear ? year - currentYear : 0;
//   });

//   const projectYear = getValues("project_completion_year_1");

//   const projectCapacity = getValues("capacity_1");
//   const handleExport = () => {
...
//         className="mb-3 px-4 py-2 bg-green-600 text-white rounded shadow"
//       >
//         Export to Excel
//       </button>
...
//             <th className="border px-3 py-2">Evaluation Parameters</th>
//             <th className="border px-3 py-2">Max Score</th>
//             <th className="border px-3 py-2">Sub KPI Score</th>
//             <th className="border px-3 py-2">Eval. Method</th>
//             <th className="border px-3 py-2">Unit of Eval</th>
//             <th className="border px-3 py-2">Minimum</th>
//             <th className="border px-3 py-2">Maximum</th>
//             {companies.map((company) => (
//               <th key={company} className="border px-3 py-2">
//                 {company}
//               </th>
//             ))}
//           </tr>
...
//             <td className="border px-3 py-2">Investment in Green Hydrogen</td>
...
//             <td className="border px-3 py-2"></td>
//             <td className="border px-3 py-2"></td>
//             {getValues("investment_activities_2023").map((v, i) => (
//               <td key={i} className="border px-3 py-2">
//                 {v.toFixed(2)}
//               </td>
//             ))}
...
//             <td className="border px-3 py-2"></td>
//             {section3.map((s, i) => (
//               <td className="border px-3 py-2">{v.toFixed(2)}</td>
//             ))}
//           </tr>
//           <tr>
...
//           <tr>
//             <td className="border px-3 py-2">5</td>
//             <td className="border px-3 py-2">Upcoming Projects</td>
//             <td className="border px-3 py-2">20</td>
//             <td className="border px-3 py-2">20</td>
//             <td className="border px-3 py-2"></td>
//             <td className="border px-3 py-2"></td>
...
//             <td className="border px-3 py-2">Project Size - Progress Status</td>
//             <td className="border px-3 py-2"></td>
//             <td className="border px-3 py-2">10</td>
//             <td className="border px-3 py-2"></td>
//             <td className="border px-3 py-2"></td>
...
const GHscoretable = () => {
  ...
  const safeFmt = (v, digits = 2) => {
    ...
    // If it's already a number and finite, format it
    ...
    if (!Number.isNaN(num) && Number.isFinite(num)) {
      // return fixed digits (or integer if digits === 0)
      return digits === 0 ? String(Math.round(num)) : num.toFixed(digits);
    }
    // For non-numeric strings or objects, return readable string
    try {
      return String(v);
    } catch {
      return "-";
  ...
  const investment_production = staticData.map(item => item.installed_capacity_2024);
  const investment_production_score = staticData.map(item => item.installed_capacity_2024);
  const growth_investment_2024 = staticData.map(item => item.investment_activities_2024);
  const growth_investment_score = staticData.map(item => item.installed_capacity_2024);
  const growth_investment_2023 = staticData.map(item => item.investment_activities_2023);
  const investment_electrolyser = staticData.map(item => item.investment_electrolyser_2024);
  const investment_electrolyser_score = staticData.map(item => item.installed_capacity_2024);
  const growth_electrolyser = staticData.map(item => item.installed_capacity_2024);
  const growth_electrolyser_score = staticData.map(item => item.installed_capacity_2024);
  ...
  const patent_commercial_score = staticData.map(item => item.instapatents_commercialized_2024_scorelled_capacity_2024);
  const project_formula = staticData.map(item => item.installed_capacity_2024);
  const project_completed_year = staticData.map(item => item.installed_capacity_2024);
  const project_completion_year = staticData.map(item => item.installed_capacity_2024);
  const project_capacity = staticData.map(item => item.installed_capacity_2024);
  const project_status = staticData.map(item => item.installed_capacity_2024);
  const Total_score = staticData.map(item => item.installed_capacity_2024);
  const installed_capacity_growth = staticData.map(item => item.installed_capacity_gh_p_unit);
  const investment_growth = staticData.map(item => item.installed_capacity_2024);
  const investment_electrolyser_growth = staticData.map(item => item.installed_capacity_2024);
  const r_n_d_growth = staticData.map(item => item.r_and_d_gh_production_total);
  const upcoming_project_growth = staticData.map(item => item.project_score);
      ...
      <table ref={tableRef} className="min-w-full border border-gray-300 shadow-md rounded-lg">
        ...
        <strong>Installed Capacity of Green hydrogen production units (MT) Score</strong>
          ...
          <tr className="hover:bg-gray-100">
            ...
            {investment_electrolyser.map((v, i) => (
              <td key={i} className="border px-3 py-2">
                {safeFmt(v, 2)}
              </td>
            ))}
