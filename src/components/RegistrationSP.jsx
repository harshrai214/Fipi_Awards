...
const RegistrationSP = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    comment: '',
    // Quantitative fields from Part 1
    // projectsAwarded: ['', ''], // [2024-25, 2023-24]
    // projectsValue: ['', ''],   // [2024-25, 2023-24]
    ...
    projectsInProgress: '',  // [2024-25, 2023-24]
    projectsInProgressRemarks: '',
    projectsInProgressValue: '', // [2024-25, 2023-24]
    projectsCompleted: '',  // [2024-25, 2023-24]
    projectsNoTimeOverrun: '', // [2024-25, 2023-24]
    projectsNoCostOverrun: '',  // [2024-25, 2023-24]
    technologiesIntroduced: '', // [2024-25, 2023-24]
    technologiesValue: '', // [2024-25, 2023-24]
    fatalities: '', // [2024-25, 2023-24]
    lostTimeInjuries: '', // [2024-25, 2023-24]
    oshaIncidents: '', // [2024-25, 2023-24]
    manHoursOwn: '',  // [2024-25, 2023-24]
    manHoursContract: '',  // [2024-25, 2023-24]
    // New Projects (Board Approved)
    ongoingProject1Name: '',
    ongoingProject1Client: '',
    ongoingProject1Start: '',
    ongoingProject1End: '',
    ...
    useEffect(() => {
      ...
      try {
        const prefillRaw = sessionStorage.getItem('registrationSP_prefill');
        if (prefillRaw) {
          ...
          sessionStorage.removeItem('registrationSP_prefill');
        }
      } catch (err) {
        // ignore parse errors
      }
  ...
  const clearFieldError = useCallback((field) => {
    ...
    useEffect(() => {
      try {
        const draftRaw = localStorage.getItem('registrationSPDraft');
        if (draftRaw) {
  ...
  const handleChange = (name, value, index = null) => {
    if ([1, 2, 3, 4].includes(currentStep)) {
      ...
      else if (['comment', 'project1_remarks', 'project2_remarks', 'project3_remarks', 'project4_remarks', 'project5_remarks'].includes(name)) {
    ...
    // Name validation
    if (['Organisationname', 'authorityName', 'contactName', 'project1_name', 'project2_name', 'project3_name', 'project4_name', 'project5_name'].includes(name)) {
    ...
    // Number field validation for quantitative fields and project capex
    if (['projectsAwarded', 'projectsValue', 'projectsInProgress', 'projectsInProgressValue', 'projectsCompleted', 'projectsNoTimeOverrun', 'projectsNoCostOverrun', 'technologiesIntroduced', 'technologiesValue', 'fatalities', 'lostTimeInjuries', 'oshaIncidents', 'manHoursOwn', 'manHoursContract', 'project1_capex', 'project2_capex', 'project3_capex', 'project4_capex', 'project5_capex'].includes(name)) {
      if (index !== null || name.includes('capex')) {
    ...
    if (index !== null) {
      setFormData((prev) => {
        const updatedArray = Array.isArray(prev[name]) ? [...prev[name]] : ['', ''];
    ...
    const printContent = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
          ...
          <tbody>
            ${part1.map(([num, title, key]) => `
              <tr>
                <td style="border: 1px solid #000; padding: 8px;">${num}</td>
                <td style="border: 1px solid #000; padding: 8px;">${title}</td>
                <td style="border: 1px solid #000; padding: 8px;">${Array.isArray(formData[key]) ? formData[key][0] || '' : formData[key] || ''}</td>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
          ...
          <tbody>
            ${ongoingProjects.map(([num, name, client, start, end, awardValue, actualCost]) => `
              <tr>
                <td style="border: 1px solid #000; padding: 8px;">${num}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[name] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[client] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[start] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[end] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[awardValue] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[actualCost] || ''}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
          ...
          <tbody>
            ${newTechnologies.map(([num, tech, project, client, cost, value]) => `
              <tr>
                <td style="border: 1px solid #000; padding: 8px;">${num}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[tech] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[project] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[client] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[cost] || ''}</td>
                <td style="border: 1px solid #000; padding: 8px;">${formData[value] || ''}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  const handleSaveDraft = () => {
    try {
      ...
      localStorage.setItem('registrationSPDraft', JSON.stringify(draftToStore));
      alert('Draft Saved!');
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Quantitative Part 1
    const quantFields = [
      'projectsAwarded', 'projectsValue', 'projectsInProgress', 'projectsInProgressValue',
      'projectsCompleted', 'projectsNoTimeOverrun', 'projectsNoCostOverrun',
      'technologiesIntroduced', 'technologiesValue', 'fatalities', 'lostTimeInjuries',
      'oshaIncidents', 'manHoursOwn', 'manHoursContract'
    ];
    quantFields.forEach((key) => {
      fd.append(`${key.replace(/([A-Z])/g, '_$1').toLowerCase()}_2024_25`, formData[key][0] || '');
      fd.append(`${key.replace(/([A-Z])/g, '_$1').toLowerCase()}_2023_24`, formData[key][1] || '');
    });
    fd.append('projects_in_progress_remarks', formData.projectsInProgressRemarks || '');

    // New Projects
    [1, 2, 3, 4, 5].forEach((i) => {
      fd.append(`project${i}_name`, formData[`project${i}_name`] || '');
      fd.append(`project${i}_board_approval_date`, formData[`project${i}_boardApprovalDate`] || '');
      fd.append(`project${i}_start_date`, formData[`project${i}_startDate`] || '');
      fd.append(`project${i}_completion_date`, formData[`project${i}_completionDate`] || '');
      fd.append(`project${i}_capex`, formData[`project${i}_capex`] || '');
      fd.append(`project${i}_remarks`, formData[`project${i}_remarks`] || '');
    });
    ...
    // POST (Commented out as per original code)
    try {
      // const url = `${ACTIVE_API_BASE_URL}/service-provider/`;
      ...
      try {
        localStorage.removeItem('registrationSPDraft');
      } catch (err) {
        // ignore
      }
    } catch (err) {
  ...
  const part1 = [
    ['1', 'No. of Projects awarded', 'projectsAwarded2024', 'projectsAwarded2023'],
    ['2', 'Value of Projects awarded', 'projectsValue2024', 'projectsValue2023'],
    ['3', 'No. of Projects in Progress', 'projectsInProgress'],
    ['4', 'Total Value of Projects in Progress (INR Crore)', 'projectsInProgressValue'],
    ['5', 'No. of Projects completed', 'projectsCompleted'],
    ['6', 'No. of Projects Completed without time overrun', 'projectsNoTimeOverrun'],
    ['7', 'No. of Projects Completed without cost overrun', 'projectsNoCostOverrun'],
    ['8', 'Number of new technologies introduced', 'technologiesIntroduced'],
    ['9', 'Value of the new technologies introduced (INR Crore)', 'technologiesValue'],
    ['10', 'No. of Fatalities', 'fatalities'],
    ['11', 'Number of lost time injuries', 'lostTimeInjuries'],
    ['12', 'Number of OSHA recordable incidents', 'oshaIncidents'],
    ['13', 'Total Man Hours Worked (Own Employees)', 'manHoursOwn'],
    ['14', 'Total Man Hours Worked (Contractual Employees)', 'manHoursContract'],
  ];
  const ongoingProjects = [
    ['1', 'ongoingProject1Name', 'ongoingProject1Client', 'ongoingProject1Start', 'ongoingProject1End', 'ongoingProject1AwardValue', 'ongoingProject1ActualCost'],
    ['2', 'ongoingProject2Name', 'ongoingProject2Client', 'ongoingProject2Start', 'ongoingProject2End', 'ongoingProject2AwardValue', 'ongoingProject2ActualCost'],
    ['3', 'ongoingProject3Name', 'ongoingProject3Client', 'ongoingProject3Start', 'ongoingProject3End', 'ongoingProject3AwardValue', 'ongoingProject3ActualCost'],
    ['4', 'ongoingProject4Name', 'ongoingProject4Client', 'ongoingProject4Start', 'ongoingProject4End', 'ongoingProject4AwardValue', 'ongoingProject4ActualCost'],
    ['5', 'ongoingProject5Name', 'ongoingProject5Client', 'ongoingProject5Start', 'ongoingProject5End', 'ongoingProject5AwardValue', 'ongoingProject5ActualCost'],
  ];

  const newTechnologies = [
    ['1', 'technology1Tech', 'technology1Project', 'technology1Client', 'technology1Cost', 'technology1Value'],
    ['2', 'technology2Tech', 'technology2Project', 'technology2Client', 'technology2Cost', 'technology2Value'],
    ['3', 'technology3Tech', 'technology3Project', 'technology3Client', 'technology3Cost', 'technology3Value'],
    ['4', 'technology4Tech', 'technology4Project', 'technology4Client', 'technology4Cost', 'technology4Value'],
    ['5', 'technology5Tech', 'technology5Project', 'technology5Client', 'technology5Cost', 'technology5Value'],
  ];

  const hasEmptyFieldsStep3 = () => {
    for (const section of part1) {
      const [num, label, key] = section;

      // Skip if no key (like headings or labels)
      if (!key) continue;

      const val = formData[key];

      // ✅ If it's undefined, null, or empty string, it's empty
  ...
  const hasEmptyFieldsStep4 = () => {
    // ✅ Check Ongoing Projects table
    for (const [num, name, client, start, end, awardValue, actualCost] of ongoingProjects) {
      const fields = [name, client, start, end, awardValue, actualCost];
    ...
    // ✅ Check New Technologies table
    for (const [num, tech, project, client, cost, value] of newTechnologies) {
      const fields = [tech, project, client, cost, value];
      ...
      <div className="form-step" role="form" aria-label={`Step ${currentStep} of 5`}>
        ...
        {currentStep === 2 && (
          <>
            ...
            <div className="form-group">
              <label htmlFor="companyProfile">Brief write up on company profile and the activities specific to service provided in the year 2024-25.</label>
        ...
        {currentStep === 3 && (
          <>
            ...
            <table className="quant-table">
              ...
              <tbody>
                {part1.slice(0, 2).map(([num, title, key2024, key2023]) => (
                  <tr key={num}>
...
/>
            ...
            <table className="quant-table">
              ...
              <tbody>
                {part1.slice(2).map(([num, title, key2024]) => (
                  <tr key={num}>
                    <td className="sno-cell">{num}</td>
                    <td className="label-cell">{title}</td>
                    <td>
...
/>
        ...
        {currentStep === 4 && (
          <>
            ...
            <table className="quant-table">
              <caption className="table-header">Table 1: Ongoing Projects Information</caption>
              <thead>
                <tr>
                  <th>S.No</th>
                  <th>Project Name</th>
            ...
            <br />
            <table className="quant-table">
              <caption className="table-header">Table 2: List of New Technologies</caption>
              <thead>
                <tr>
                  <th>S.No</th>
